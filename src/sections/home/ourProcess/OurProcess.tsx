    ...
    return (
        <div className='container mt-[100px]'>
            <div className='max-w-[800px] mx-auto'>
                <SubHeading className={'text-center'} subHeading={'Ideation to Execution'} />
                <GeneralHeading className={'text-center'} heading={'Our process. Simple, seamless, streamlined.'} />
            ...
            <div className='grid md:grid-cols-2 grid-cols-1 mt-[60px] gap-[50px]'>
                <div className={`relative ${styles.imageSection}`}>
                    ...
                    <div>     <Image src={businemanImage} alt="thinking busineman image" /> </div>
                </div>
                        ...
                        OUR_PROCESS_DATA.map((data: any, index: number) => {
                            return (
                                <div className='flex gap-[30px] mb-[67px]' key={index}>
                                    ...
                                    <div>
                                        <h5 className='text-sm	font-bold	font-poppins  uppercase text-grey-100 tracking-[2px] mb-[2px]'>{data.subtile}</h5>
                                        <h3 className='text-xl font-bold text-primary-dark	font-poppins	mb-[16px]'> {data.title}</h3>
                                        <p className='text-lg font-normal text-grey	font-poppins max-w-[460px]'>{data.text}</p>
                                    </div>
                                </div>
                            )
                        })
