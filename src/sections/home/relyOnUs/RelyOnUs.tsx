                ...
                <div className='grid grid-col-1 md:grid-cols-2'>
                    <div>
                        <div className='max-w-[555px]'>
                            <SubHeading className={'text-left mb-[8px]'} subHeading={'Rely on Us'} />
                            <GeneralHeading className={'text-left md:leading-[64px] leading-[35px]'} heading={'500+ companies rely on us to scale their dev teams.'} />
                        </div>
                    </div>
                                ...
                                COMPANIES_DATA.map((data: any, index) => {
                                    return (
                                        <div className='mb-8 text-center' key={index}>
                                            <Image src={data.image} className="mx-auto" alt={data.title} height={50} />
                                            <a className='font-poppins text-xs 	text-purple font-normal	flex items-center justify-center gap-2 cursor-pointer'>{data.link} {data.playIcon} </a>
                                        </div>
                                    )
                                })
                            }
